...
export default function TacticalBoard() {
  ...
  const measure = useCallback(() => {
    ...
    const mirrored = FORMATION.map((p) => ({ ...p, id: `OPP_${p.id}`, x: (1 - p.xPct) * w, y: (1 - p.yPct) * h }));
  ...
  const onOppositionDragStop = (posId, _, data) => {
    const jerseyHalf = jerseySize / 2;
    setOppositionPlayers((ps) => ps.map((p) => p.id === posId ? { ...p, x: data.x + jerseyHalf, y: data.y + jerseyHalf } : p));
  };
  ...
  return (
    <div style={{ width: "100vw", height: "100dvh", background: "#020617", position: "fixed", inset: 0, display: "flex", flexDirection: isMobile ? "column" : "row" }}>
      ...
      <div ref={wrapperRef} style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", padding: "12px", overflow: "hidden" }}>
        {ready && w > 0 && (
          <div ref={pitchRef} style={{ width: `${w}px`, height: `${h}px`, borderRadius: "20px", overflow: "hidden", position: "relative", border: "4px solid #e2e8f0", background: "#1a3a1a", touchAction: "none" }} onMouseDown={onDrawStart} onMouseMove={onDrawMove} onMouseUp={onDrawEnd} onMouseLeave={onDrawEnd} onTouchStart={onDrawStart} onTouchMove={onDrawMove} onTouchEnd={onDrawEnd}>
            {[...Array(20)].map((_, i) => (<div key={i} style={{ position: "absolute", top: `${i * (100 / 20)}%`, width: "100%", height: `${100 / 20}%`, background: i % 2 === 0 ? "#166534" : "#15803d" }} />))}
            ...
            <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ position: "absolute", inset: 0 }}>
              ...
              {lines.map((l, i) => (<line key={i} x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2} stroke="#fde047" strokeWidth="5" markerEnd="url(#arrow)" />))}
              {drawing && (<line x1={drawing.x1} y1={drawing.y1} x2={drawing.x2} y2={drawing.y2} stroke="#fde047" strokeWidth="5" strokeDasharray="8 5" />)}
            </svg>
